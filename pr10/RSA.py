...
for msg_str in ciphers:
    try:
        encrypted_blob = base64.b64decode(msg_str)
        decrypted = private_key.decrypt(encrypted_blob)
        print(decrypted)
    except:
        pass

#answer is: b'I love cryptography still'
...
for elements in messages:
    ...
    signature_bytes = base64.b64decode(elements["signature"])
    text = elements["text"]
    hash = int.from_bytes(sha512(text).digest(), byteorder='big')
    hashFromSignature = pow(int.from_bytes(signature_bytes, byteorder='big'), pubKey.e, pubKey.n)
    if (hash == hashFromSignature):
        print(text)


#answer 'The code is zhijc'
