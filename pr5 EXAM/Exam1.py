...
#
# The assignment is to build up non-secure encryption schemes.
...
#
# You not allowed to use external library, you can use the solution codes from the canvas, but copy paste all of the code that you used and refere, where you copeid from.
...
#
# If you have any problem/question write on teams or email.
#
...
def encrypt_with_one_byte_prg(plaintext, key, key_multiplier, key_increment):
    ...
    'Cryptography'
    '''
    #We initialize the resultat to an empty string for using it later
    res = ""
    #we iterate for every letter in the plaintext (each letter = 1 byte)
    for element in plaintext:
        #transforming the letter to its unicode number
        charBin = ord(element)

        #performing the xor and modulo
        result = (charBin ^ key)%256

        #converting back the result to ascii
        result = chr(result)

        #Adding the new letter to the result
        res = res + result

        #new_key = last_key * key_multiplier + key_increment.
        key = (key*key_multiplier)+ key_increment
    return res
...
def base64_to_hex(base):
    ...
    '11eeaa55ccdd'
    '''
    #using only the basic function from the library base64
...
def swap_lower_and_upper_bits(input):
    ...
    '0b10101001'
    '''
    #modifying the input in binary and removing the "ob"
    binaryValue =(bin(input))[2:]

    #getting the length of the binary result
    length = len(binaryValue)

    #We iterate for each byte
    for _ in range (int(length/8)+1):

        #if byte does not have 8 bits, we make sure to create an 8 bit string
        if(length <= 8):
            for _ in range (8 - length):
                binaryValue = "0" + binaryValue
        
        #we change the first 4 bits to be the last 4 bits
        newbin = binaryValue[4:] + binaryValue[:4]
    #we retrun an int
    return int(newbin,2)
...
def score_text(most_used_letters,text):
    ...
    8
    '''
    #we need to initialise count for the first iteration
    count = 0

    #we loop for every letter in the text
    for element in text:

        #we check if the letter is in the most_used_letters string and if so, we had it to the count
...
def decrypt_single_byte_xor(cipher):
    ...
    res = ""
    check = 0
    # we test every possible one byte option for the xor key
    for currentKey in range(256):
        currentDecrypt = encrypt_single_byte_xor(cipher, hex(currentKey)[2:])

        #we score the text
        counter = score_text(most_used_letters, hex2string(currentDecrypt))

        #if the score is higher than the last one, we have a better solution and we make it the result
        if counter > check:
            res = hex2string(currentDecrypt)
            check = counter
    return (res, check)
...
def find_correct_row_in_base64_encoded(base64_encoded_ciphers):
    ...
    most_used_letters = 'etaoin'
    check = 0
    #we iterate for each element in the list
    for element in base64_encoded_ciphers:
        #we decrypt the element using both previous function (tranforming from base64 to hex and then we decrypt it)
        currentDecryption = (decrypt_single_byte_xor(base64_to_hex(element))[0])
        #if the score is higher than the last one, we have a better solution and we make it the result
        counter = score_text(most_used_letters,currentDecryption )
        if counter > check:
            res = currentDecryption
            check = counter
